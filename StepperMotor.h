...
class StepperMotor 
{
  public:
     ...
     */
    StepperMotor(string motorPin1, string motorPin2, string motorPin3, string motorPin4);  
  ...
  private:
	...
	inline void assignMotorPins()
	{
		...
		{
			gpios[n] = new GPIOClass(motorPins[n]); //create new GPIO object to be attached to GPIO port
		    gpios[n]->export_gpio(); //export GPIO
			gpios[n]->setdir_gpio("out"); //GPIO set to output
			cout << " Set GPIO pin directions" << motorPins[n] << endl;
		}
	}
    ...
    string motorPins[PHASE_NUMBER];   // Motor PIN assignment
	GPIOClass *gpios[PHASE_NUMBER];	 // IO classes
